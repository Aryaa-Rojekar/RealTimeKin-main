...
def main():
    ...
    fake_online_data = home_dir
    sto_filename = home_dir+'tiny_file.sto'
    record_filename = home_dir + 'record.txt'
    visualize = True
    ...
    while(script_live):
        ...
        oRefs = osim.OrientationsReference()
        ...
        while(running):
            ...
            if (b.qsize() > 0) or t == sim_steps: # new button press so we should save the data and restart the sim
                ...
                if log_data:
                    ...
                    print(save_dir+save_file+str(file_cnt)+'.sto')
            ...
            orientationsData = osim.OpenSenseUtilities.convertQuaternionsToRotations(quatTable)
            rowVecView = orientationsData.getNearestRow(time_s+dt)
            ...
            hip_flexion = model.getCoordinateSet().get("hip_flexion_l").getValue(s0)

            with open(record_filename, "a") as f:
                f.write(str(hip_flexion) + '\n')
